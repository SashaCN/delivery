...
class Package extends Model
{
  ...
  public function save ()
  {
    ...
    if (!empty($this->id)) {
      ...
      $sql = 'UPDATE `package` SET `Pack_type_id`=:pack_type,`Created_at`=:date_add,`User_id_from`=:user_from,`User_id_to`=:user_to,`Address_id`=:address_to,`Status`=:pack_status,`Manager_id`=:manager,`Address_from`=:address_from WHERE `Pack_id` = :pack_id';
    } else {
      $sql = 'INSERT INTO package (`pack_type_id`, `created_at`, `User_id_from`, `User_id_to`, `Address_id`, `Status`, `Manager_id`, `Address_from`) VALUES (:pack_type, :date_add, :user_from, :user_to, :address_to, :pack_status, :manager, :address_from)';
    }
